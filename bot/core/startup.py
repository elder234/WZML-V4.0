...
from os import environ, path as ospath, getenv, cpu_count
...
async def _keepalive_loop(initial_url):
...
async def load_configurations():
    ...
    if PORT:
        ...
        web_workers = max(2, min(4, (cpu_count() or 2) // 4))
        bot_loop.create_task(cmd_exec(
            f"{env}gunicorn -k uvicorn.workers.UvicornWorker -w {web_workers} web.wserver:app --bind 0.0.0.0:{PORT}",
            shell=True,
        ))
