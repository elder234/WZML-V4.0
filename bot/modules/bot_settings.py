...
DEFAULT_DESP = {
    ...
    "CF_WORKER_URL": "Cloudflare Worker hostname for Real-Debrid link proxying (e.g. alphadebrid.yourname.workers.dev).",
    "CF_WORKER_KEY": "32-character AES key shared between bot and CF Worker. Must match the API_KEY secret on the worker.",
    ...
    "MAX_CONCURRENT_UPLOADS": "Max parallel Telegram file uploads per task. 0 = auto (helper bot count).",
    "MAX_CONCURRENT_EPISODES": "Max anime episodes processed in parallel. 0 = auto (helper bot count).",
    "RENAME_ENABLED": "Auto-rename media files using RENAME_TEMPLATE. Default: False.",
    "RENAME_TEMPLATE": "Rename template. Variables: {title} {year} {season} {episode} {resolution} {source} {language} {audio} {codec} {uploader} {ext}.",
    "RENAME_UPLOADER": "Uploader name inserted into rename template {uploader}.",
    "ANIWATCH_BASE": "AniWatch site base URL for the anime scraper.",
    "ANIME_TOKI_BASE": "AnimeToki site base URL for the anime scraper.",
    "ANIME_DEFAULT_CATEGORY": "Default audio category for anime: sub or dub.",
...
async def edit_variable(_, message, pre_message, key):
    ...
    if value.lower() == "true":
        ...
        value = str(value)
    elif key == "CF_WORKER_URL":
        value = str(value).strip("/")
    elif key == "CF_WORKER_KEY":
        value = str(value)
        if value and len(value) != 32:
            await send_message(message, "CF_WORKER_KEY must be exactly 32 characters!")
            return
    elif value.isdigit():
        value = int(value)
