...
from re import findall, match, search, IGNORECASE, MULTILINE
...
real_debrid_supported_sites = [
    ...
    "hitfile.net", "hotlink.cc", "hulkshare.com", "icerbox.com",
    "inclouddrive.com", "isra.cloud", "katfile.com", "keep2share.cc",
    "letsupload.cc", "load.to", "down.mdiaload.com", "mediafire.com",
    "mega.co.nz", "mixdrop.co", "mixloads.com", "mp4upload.com", "nelion.me",
    "ninjastream.to", "nitroflare.com", "nowvideo.club", "oboom.com",
    "prefiles.com", "sky.fm", "rapidgator.net", "rapidrar.com", "rapidu.net",
    "rarefile.net", "real-debrid.com", "redbunker.net", "redtube.com",
    "rockfile.eu", "rutube.ru", "scribd.com", "sendit.cloud", "sendspace.com",
    ...
    "uppit.com", "upstore.net", "upstream.to", "uptobox.com",
    "userscloud.com", "usersdrive.com", "vidcloud.ru", "videobin.co",
    "vidlox.tv", "vidoza.net", "vimeo.com", "vivo.sx", "vk.com", "voe.sx",
    "wdupload.com", "wipfiles.net", "world-files.com", "worldbytez.com",
    "wupfile.com", "wushare.com", "xubster.com", "youporn.com",
]
...
def _rd_encrypt(data: str) -> str:
    """AES-CBC encrypt a string for the CF Worker."""
    cyp = AES.new(Config.CF_WORKER_KEY.encode("utf-8"), AES.MODE_CBC)
    encrypted = cyp.encrypt(pad(data.encode("utf-8"), cyp.block_size))
    payload = b64encode(cyp.iv).decode() + b64encode(encrypted).decode()
...
def _rd_api_call(method: str, path: str, body: str = None) -> dict:
    """Call Real-Debrid API directly through WARP (same IP as aria2).
    No CF Worker proxy needed — keeps it fast and avoids HTTP/3 issues."""
    headers = {
        "Authorization": f"Bearer {Config.RD_TOKEN}",
    }
    ...
    if not resp.ok:
        try:
            ...
            err = resp.text
        raise DirectDownloadLinkException(f"ERROR: Real-Debrid: {err}")
    if not resp.content:
        return {}
    return resp.json()


def _rd_unrestrict_direct(link: str) -> str:
    """Call RD unrestrict/link directly through WARP (not via Worker).
    This ensures the download URL is locked to the same IP aria2 uses."""
    resp = post(
    ...
    if not download_url:
        raise DirectDownloadLinkException("ERROR: Real-Debrid returned no download URL")
    return download_url


def real_debrid(url):
    """Unrestrict a hoster link via RD API (through WARP), then encrypt+wrap
    in a CF Worker URL. Worker decrypts and 302 redirects. aria2 (on WARP)
    follows the redirect — same IP for both unrestrict and download.
    Requires RD_TOKEN and CF_WORKER_URL/CF_WORKER_KEY to be set in config.
    """
    if not Config.RD_TOKEN:
        raise DirectDownloadLinkException("ERROR: RD_TOKEN must be set in config")
    if not Config.CF_WORKER_URL or not Config.CF_WORKER_KEY:
        raise DirectDownloadLinkException("ERROR: CF_WORKER_URL and CF_WORKER_KEY must be set in config")
...
def real_debrid_torrent(magnet_or_path: str) -> dict:
    """Add a magnet or torrent file to Real-Debrid directly via RD API.
    Returns a dict with 'contents' and 'title' for multi-file or a direct URL string.
    Requires RD_TOKEN to be set in config.
    """
    if not Config.CF_WORKER_URL or not Config.CF_WORKER_KEY:
        raise DirectDownloadLinkException(
            "ERROR: CF_WORKER_URL and CF_WORKER_KEY must be set in config for torrent flow"
        )
    if len(Config.CF_WORKER_KEY) != 32:
        raise DirectDownloadLinkException(
            "ERROR: CF_WORKER_KEY must be exactly 32 characters"
        )
    if not Config.RD_TOKEN:
        raise DirectDownloadLinkException(
            "ERROR: RD_TOKEN must be set in config for direct unrestrict"
        )
    ...
    if magnet_or_path.startswith("magnet:"):
        data = _rd_api_call(
            ...
            urlencode({"magnet": magnet_or_path}),
        )
    elif isfile(magnet_or_path):
        # Upload torrent file bytes encoded as base64 in the body
        from base64 import b64encode as _b64e
        with open(magnet_or_path, "rb") as f:
            torrent_b64 = _b64e(f.read()).decode()
        data = _rd_api_call(
            "POST",
            "/rest/1.0/torrents/addTorrent",
            urlencode({"torrent": torrent_b64}),
        )
    else:
        raise DirectDownloadLinkException("ERROR: Not a magnet link or torrent file path")

    torrent_id = data.get("id")
    if not torrent_id:
        raise DirectDownloadLinkException("ERROR: Real-Debrid did not return torrent ID")

    # Step 2: Smart file selection — keep only real episodes/movies
    info = _rd_api_call("GET", f"/rest/1.0/torrents/info/{torrent_id}")
    all_files = info.get("files", [])

    from re import compile as _rc

    _VIDEO_EXTS = (".mkv", ".mp4", ".avi", ".mov", ".wmv", ".flv", ".webm", ".ts", ".m2ts")

    # Patterns that identify real content (episodes/movies with resolution tags)
    _EPISODE_RE = _rc(r"[Ss]\d{1,2}[Ee]\d{1,2}")
    _ANIME_EP_RE = _rc(r"[\s\-_]+(?:EP?|Episode|#)?\s*\d{2,4}(?:\s*[\[\(v]|\.mkv|\.mp4)", IGNORECASE)
    _BATCH_RE = _rc(r"[\s\-_]+\d{2,3}\s*[\[\(]")  # "- 01 [720p]" style
    _RESOLUTION_RE = _rc(r"(2160|1080|720|480|360)[pi]")
    _CODEC_RE = _rc(r"(x26[45]|h\.?26[45]|hevc|avc|av1)", IGNORECASE)
    _SOURCE_RE = _rc(r"(bluray|blu-ray|bdrip|brrip|web-?dl|webrip|hdtv|dvdrip|remux)", IGNORECASE)

    # Folders/paths that indicate extras (not real content)
    _EXTRAS_FOLDERS = (
        "featurette", "extras", "bonus", "sample", "behind the scenes",
        "deleted scene", "interview", "trailer", "special feature",
        "making of", "blooper", "gag reel", "commentary", "promo",
        "extra", "bts", "short",
    )

    def _is_real_content(fpath: str) -> bool:
        """Returns True if the file looks like an actual episode or movie."""
        fname = fpath.rsplit("/", 1)[-1]
        path_lower = fpath.lower()
        # Exclude if in an extras folder
        for folder in _EXTRAS_FOLDERS:
            if f"/{folder}/" in path_lower or path_lower.startswith(f"{folder}/"):
                return False
        # Must be a video file
        if not any(fname.lower().endswith(ext) for ext in _VIDEO_EXTS):
            return False
        # Match standard episode pattern (S01E01)
        if _EPISODE_RE.search(fname):
            return True
        # Match anime-style episodes: "- 01 [1080p]", "- 001.mkv", "EP01", "Episode 12"
        if _ANIME_EP_RE.search(fname) or _BATCH_RE.search(fname):
            return True
        # Match if it has resolution + codec/source (movie pattern)
        if _RESOLUTION_RE.search(fname) and (_CODEC_RE.search(fname) or _SOURCE_RE.search(fname)):
            return True
        # Match if resolution alone (common for single movies and anime)
        if _RESOLUTION_RE.search(fname):
            return True
        return False
    ...
    if not selected_ids:
        selected_ids = [str(f["id"]) for f in all_files]

    _rd_api_call(
        "POST",
        f"/rest/1.0/torrents/selectFiles/{torrent_id}",
        f"files={','.join(selected_ids)}",
    )

    # Step 3: Check if already cached — skip polling if ready immediately
    ...
    if info.get("status") != "downloaded":
        for attempt in range(60):
            if info.get("status") == "downloaded":
                break
            if info.get("status") in ("error", "magnet_error", "virus", "dead"):
                raise DirectDownloadLinkException(
                    f"ERROR: Real-Debrid torrent failed: {info.get('status')}"
                )
            _sleep(1 if attempt < 5 else 3)
            info = _rd_api_call("GET", f"/rest/1.0/torrents/info/{torrent_id}")
        else:
            raise DirectDownloadLinkException(
                "ERROR: Real-Debrid torrent timed out waiting for download"
            )
    ...
    unrestricted = [None] * len(links)
    with ThreadPoolExecutor(max_workers=min(20, len(links))) as pool:
        futures = {pool.submit(_rd_unrestrict_direct, link): i for i, link in enumerate(links)}
        for future in as_completed(futures):
            idx = futures[future]
            unrestricted[idx] = future.result()
    ...
    for i, dl_url in enumerate(unrestricted):
        ...
        if i < len(selected_files):
            ...
            subdir, filename = (parts[0], parts[1]) if len(parts) == 2 else ("", parts[0])
            size = selected_files[i].get("bytes", 0)
        else:
            filename_match = findall(_fnr, links[i], IGNORECASE | MULTILINE)
            filename = filename_match[0] if filename_match else "download"
            subdir, size = "", 0
        total_size += size
        contents.append({
    ...
    return {"title": title, "total_size": total_size, "contents": contents}
