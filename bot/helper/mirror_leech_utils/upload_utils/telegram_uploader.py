...
class TelegramUploader:
    ...
    async def _upload_file_task(self, file_, f_path, dirpath, user_session, seq_idx):
        up_path = None
        try:
            if self._listener.is_cancelled or not await aiopath.exists(f_path):
                return None
            ...
            if not self._listener.is_cancelled:
                LOGGER.warning(f"File vanished during upload: {f_path}")
            return None
        except Exception as err:
            ...
            for file_ in natsorted(files):
                ...
                if f_size == 0:
                    ...
                    if not self._listener.is_cancelled:
                        await remove(f_path)
                    continue
                all_upload_files.append((f_size, file_, f_path, dirpath))
    ...
    async def _upload_file(
        ...
        try:
            ...
            if self._listener.is_cancelled or not await aiopath.exists(o_path):
                return
    ...
    def speed(self):
        ...
        if elapsed >= 2:
            delta_bytes = self._processed_bytes - self._last_speed_bytes
            self._last_speed_bytes = self._processed_bytes
            self._last_speed_time = now
            self._current_speed = delta_bytes / elapsed if elapsed > 0 else 0
        if not hasattr(self, '_current_speed') or self._current_speed <= 0:
            # Fallback to average speed at start
            try:
