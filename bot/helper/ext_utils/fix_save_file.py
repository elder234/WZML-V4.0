...
async def _save_file_patched(
    ...
    async with self.save_file_semaphore:
        ...
        async def worker(idx):
            while True:
                ...
                for attempt in range(MAX_RETRIES):
                    s = pool[(idx + attempt) % len(pool)]
                    try:
                        await asyncio.wait_for(s.invoke(data), timeout=PART_TIMEOUT)
                        break
        ...
        is_premium = (
            self.me.is_premium if hasattr(self.me, "is_premium") else False
        )
        if is_bot:
            rate_limit = 40
        ...
        workers = [
            self.loop.create_task(worker(i)) for i in range(n_workers)
        ]
        ...
        try:
            ...
            while True:
                ...
                for chunk in chunks:
                    ...
                    _next_dispatch = (
                        max(time.monotonic(), _next_dispatch) + _dispatch_interval
                    )
...
def apply_save_file_patch():
    ...
    setattr(Client.save_file, "_wzmlx_patched", True)
    log.warning(
        "Applied wzgram save_file patch: no producer timeout, "
        "per-part invoke timeout %ss",
        PART_TIMEOUT,
    )
