...
async def get_readable_message(sid, is_user, page_no=1, status="All", page_step=1):
    ...
    for index, task in enumerate(
        ...
        msg += f"\n\n👤 {task.listener.message.from_user.mention(style='html')}"
        ...
        if (
            ...
            if task.listener.subname:
                ...
                subsize = ""
                count = ""
            msg += f"\n📦 {task.processed_bytes()}{subsize} of {task.size()}{count}"
            msg += f"\n⚡ {task.speed()}  •  ⏳ {task.eta()}"
            msg += f"  •  ⏱ {get_readable_time(elapsed)}"
            ...
            msg += f"\n📦 {task.size()} • ⬆ {task.uploaded_bytes()}"
            msg += f"\n⚡ {task.seed_speed()}  •  📊 {task.ratio()}"
            msg += f"\n⏱ {task.seeding_time()} • Elapsed: {get_readable_time(elapsed)}"
        else:
            msg += f"\n📦 {task.size()}"
    ...
    if len(tasks) > STATUS_LIMIT:
        msg += f"\n📄 Page {page_no}/{pages}  •  📋 Tasks {tasks_no}  •  Step {page_step}"
