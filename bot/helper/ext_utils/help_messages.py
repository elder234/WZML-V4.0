...
def get_bot_commands():
    ...
    static_commands = {
        ...
        "Anime": "[query] Search & download anime from AniWatch",
...
def get_help_string():
    ...
    help_lines = ["📖 <b>Bot Commands</b>\n<i>Try each command without arguments for details.</i>\n"]
    ...
    for key, cmds in commands.items():
        ...
        if key == "Mirror":
            help_lines.append(f"📥 {cmd_str}: Mirror to cloud")
        elif key == "QbMirror":
            help_lines.append(f"🧲 {cmd_str}: Mirror via qBittorrent")
        elif key == "JdMirror":
            help_lines.append(f"⬇ {cmd_str}: Mirror via JDownloader")
        elif key == "NzbMirror":
            help_lines.append(f"📰 {cmd_str}: Mirror via SABnzbd")
        elif key == "Ytdl":
            help_lines.append(f"▶ {cmd_str}: Mirror yt-dlp link")
        elif key == "UpHoster":
            help_lines.append(f"🌐 {cmd_str}: Upload to DDL servers")
        elif key == "Leech":
            help_lines.append(f"📤 {cmd_str}: Leech to Telegram")
        elif key == "QbLeech":
            help_lines.append(f"🧲 {cmd_str}: Leech via qBittorrent")
        elif key == "JdLeech":
            help_lines.append(f"⬇ {cmd_str}: Leech via JDownloader")
        elif key == "NzbLeech":
            help_lines.append(f"📰 {cmd_str}: Leech via SABnzbd")
        elif key == "YtdlLeech":
            help_lines.append(f"▶ {cmd_str}: Leech yt-dlp link")
        elif key == "Clone":
            help_lines.append(f"📋 {cmd_str}: Clone to Google Drive")
        elif key == "Count":
            help_lines.append(f"🔢 {cmd_str}: Count GDrive files")
        elif key == "Delete":
            help_lines.append(f"🗑 {cmd_str}: Delete from GDrive")
        elif key == "UserSet":
            help_lines.append(f"⚙️ {cmd_str}: User settings")
        elif key == "BotSet":
            help_lines.append(f"🔧 {cmd_str}: Bot settings")
        elif key == "Select":
            help_lines.append(f"📂 {cmd_str}: Select torrent/nzb files")
        elif key == "CancelTask":
            help_lines.append(f"❌ {cmd_str}: Cancel task")
        elif key == "ForceStart":
            help_lines.append(f"⚡ {cmd_str}: Force start task")
        elif key == "CancelAll":
            help_lines.append(f"🚫 {cmd_str}: Cancel all tasks")
        elif key == "List":
            help_lines.append(f"🔍 {cmd_str}: Search Google Drive")
        elif key == "Search":
            help_lines.append(f"🔎 {cmd_str}: Search torrents")
        elif key == "MediaInfo":
            help_lines.append(f"🎬 {cmd_str}: Get media info")
        elif key == "Status":
            help_lines.append(f"📊 {cmd_str}: Show download status")
        elif key == "Stats":
            help_lines.append(f"📈 {cmd_str}: Show bot/OS stats")
        elif key == "Ping":
            help_lines.append(f"🏓 {cmd_str}: Ping bot")
        elif key == "Authorize":
            help_lines.append(f"✅ {cmd_str}: Authorize chat/user")
        elif key == "UnAuthorize":
            help_lines.append(f"🚫 {cmd_str}: Unauthorize chat/user")
        elif key == "Users":
            help_lines.append(f"👥 {cmd_str}: Show user settings")
        elif key == "AddSudo":
            help_lines.append(f"👑 {cmd_str}: Add sudo user")
        elif key == "RmSudo":
            help_lines.append(f"👤 {cmd_str}: Remove sudo user")
        elif key == "BlackList":
            help_lines.append(f"🚷 {cmd_str}: Blacklist user")
        elif key == "RmBlackList":
            help_lines.append(f"🔓 {cmd_str}: Remove from blacklist")
        elif key == "AddImage":
            help_lines.append(f"🖼 {cmd_str}: Add gallery image")
        elif key == "Images":
            help_lines.append(f"🎨 {cmd_str}: Manage gallery")
        elif key == "Restart":
            help_lines.append(f"🔄 {cmd_str}: Restart bot")
        elif key == "Log":
            help_lines.append(f"📋 {cmd_str}: Get bot logs")
        elif key == "Shell":
            help_lines.append(f"💻 {cmd_str}: Run shell commands")
        elif key == "AExec":
            help_lines.append(f"⚡ {cmd_str}: Exec async functions")
        elif key == "Exec":
            help_lines.append(f"⚡ {cmd_str}: Exec sync functions")
        elif key == "ClearLocals":
            help_lines.append(f"🧹 /{BotCommands.ClearLocalsCommand}: Clear locals")
        elif key == "Rss":
            help_lines.append(f"📡 /{BotCommands.RssCommand}: RSS Menu")
        elif key == "GenPyroSess":
            help_lines.append(f"🔑 /{BotCommands.GenPyroSessCommand}: Generate Pyrogram session")
