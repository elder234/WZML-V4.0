...
TRACKER_MAX_AGE=86400

if [ -f "$TRACKER_CACHE" ]; then
    cache_age=$(($(date +%s) - $(stat -c %Y "$TRACKER_CACHE" 2>/dev/null || stat -f %m "$TRACKER_CACHE" 2>/dev/null || echo 0)))
else
    cache_age=$TRACKER_MAX_AGE
fi

if [ "$cache_age" -lt "$TRACKER_MAX_AGE" ] && [ -s "$TRACKER_CACHE" ]; then
    tracker_list=$(cat "$TRACKER_CACHE")
else
    tracker_list=$(curl -Ns --connect-timeout 10 --max-time 30 https://cdn.jsdelivr.net/gh/ngosang/trackerslist@master/trackers_all.txt | awk '$0' | tr '\n\n' ',')
    if [ -n "$tracker_list" ]; then
        printf '%s' "$tracker_list" > "$TRACKER_CACHE"
    elif [ -f "$TRACKER_CACHE" ] && [ -s "$TRACKER_CACHE" ]; then
        tracker_list=$(cat "$TRACKER_CACHE")
    fi
fi
$ARIA2_CMD \
