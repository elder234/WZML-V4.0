...
def _run_update(upstream_repo, upstream_branch, version):
    if not upstream_repo:
        ...
        return
    if path.exists(".git"):
        existing_remote = srun(
            ["git", "remote", "get-url", "origin"],
            capture_output=True,
            text=True,
        )
        if (
            existing_remote.returncode == 0
            and existing_remote.stdout.strip() == upstream_repo
        ):
            _LOGGER.info("Git repo already up-to-date, fetching latest...")
            fetch_result = srun(["git", "fetch", "origin", "-q"])
            ...
            _LOGGER.info("Remote changed, re-initializing git...")
            srun(["rm", "-rf", ".git"])
            result = _init_and_fetch(upstream_repo, upstream_branch)
    else:
        result = _init_and_fetch(upstream_repo, upstream_branch)
...
def _init_and_fetch(upstream_repo, upstream_branch):
    git_cmds = [
        ["git", "init", "-q"],
        [
            "git",
...
def _update_packages():
    ...
    if path.exists(hash_file):
        ...
        if stored_hash == current_hash:
            _LOGGER.info("Packages unchanged, skipping pip install")
            return
